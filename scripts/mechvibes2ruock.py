...
def convert_to_ruock(mechvibes_data: dict) -> dict:
    """
    Convert MechVibes config dict into Ruock format.
...
def write_ruock_config(ruock_data: dict, output_dir: str):
    """
    Write Ruock config.json into output directory.
    """
    with open(os.path.join(output_dir, "config.json"), "w") as f:
        json.dump(ruock_data, f, indent=4)


def create_ruock_pack(mechvibes_path: str):
    """
    Master function to convert MechVibes config into Ruock pack folder.
    """
    mechvibes_data = load_mechvibes_config(mechvibes_path)
    ruock_data = convert_to_ruock(mechvibes_data)

    pack_name_safe = sanitize_name(ruock_data["metadata"]["name"])
    script_dir = os.path.dirname(os.path.abspath(__file__))
    sound_src_dir = get_sound_source_dir(script_dir, pack_name_safe)

    output_dir = os.path.join(script_dir, f"{pack_name_safe}_ruock_{str(uuid.uuid4())[:8]}")
    os.makedirs(output_dir, exist_ok=True)

    sounds_set = set()
    for sound_obj in ruock_data["sounds"].values():
        sounds_set.update(sound_obj.get("down", []))
        sounds_set.update(sound_obj.get("up", []))

    copy_sounds(sounds_set, sound_src_dir, output_dir)
    write_ruock_config(ruock_data, output_dir)

    print(f"Ruock soundpack created at: {output_dir}")
...
def main():
    ...
    if not os.path.isfile(mechvibes_path):
        ...
        return
    create_ruock_pack(mechvibes_path)
