...
class MenuBarController {
    ...
    @objc private func copyUpgradeCommand() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            pasteboard.setString("brew update && brew upgrade ruock", forType: .string)
    ...
    @objc private func openPermissionsDocs() {
        if let url = URL(string: "https://github.com/ded-furby/ruock#permissions") {
            NSWorkspace.shared.open(url)
        }
    }
    
    @objc private func openChangelog() {
        if let url = URL(string: "https://github.com/ded-furby/ruock/releases/tag/\(AppInfoHelper.appVersion)") {
            NSWorkspace.shared.open(url)
        }
    }
