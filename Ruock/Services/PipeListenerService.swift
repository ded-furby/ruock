...
final class PipeListenerService {
    ...
    private init() {
        let supportDir = FileManager.default
            ...
            .appendingPathComponent("Library/Application Support/ruock")
        if !FileManager.default.fileExists(atPath: supportDir.path) {
            try? FileManager.default.createDirectory(at: supportDir, withIntermediateDirectories: true)
        }
        
        self.pipePath = supportDir.appendingPathComponent("ruock.pipe").path
    ...
    private func handleSetEnabledCommand(components: [String]) {
        ...
        DispatchQueue.main.async {
            ...
            print("[COMMAND] Ruock is now \(enabled ? "enabled" : "disabled")")
        }
    }
}
